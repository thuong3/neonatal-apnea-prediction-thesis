...
def init_neonatal_dataset(pat_id, cfg):
    if cfg.meta.pickle_path is None:
        # Opt-in per cohort, and defaulting to OFF on purpose: the drift table
        # is keyed by patient id, and the Robin-sequence ids overlap the CPAP
        # ones (001-015), so defaulting to the CPAP file would silently apply
        # one cohort's clock correction to another's recordings. A dataset
        # config asks for it by name; anything that does not runs uncorrected,
        # which also keeps every pre-existing result reproducible.
        drift = load_clock_drift(cfg.dataset.get("clock_drift_file", None))
        signal_dict = read_data(
            pat_id,
            cfg.meta.data_path,
            annotations_dir=cfg.dataset.get("annotations_dir", "annotations"),
            record_duration=cfg.dataset.get("record_duration", None),
    ...
    return NeoNatal(
        ...
        # Optional; absent from every dataset config except the BA control
        # experiment, so existing runs are unaffected.
        positive_cap=cfg.dataset.get("positive_cap", None),
...
def init_classifier(cfg):
    # BA adaptation (F2): read through `.get` so every existing network config
    # keeps working untouched and unchanged runs stay reproducible.
    dropout_p = float(cfg.network.get("dropout_p", 0.0))
...
def train_model_and_evaluate_performance(
    ...
    train_loader = DataLoader(
        ...
        sampler=MultiDatasetBalancedSampler(
            train_dataset,
            replacement=False,
            # BA adaptation (F3). Absent from every dataset config except the
            # two sampler_* experiments, so existing runs are unaffected.
            equal_per_dataset=cfg.dataset.get("equal_per_patient", False),
            target_per_dataset=cfg.dataset.get("sampler_target_per_class", None),
        ),
        pin_memory=True,
    )
    ...
    for _epoch in range(cfg.optimizer.epochs):
        ...
        # BA diagnostics: make train/test curves visible in the log (wandb is
        # disabled by default). Comparing train vs test AUC tells us whether a
        # weak result is under-fitting (both low) or no generalisation (train
        # high, test ~0.5).
        log.info(
            f"  epoch {_epoch + 1}/{cfg.optimizer.epochs}: "
            f"train_auc={train_metrics['agg_train_auc']:.3f} "
            f"test_auc={test_metrics['agg_test_auc']:.3f} | "
            f"train_ap={train_metrics['agg_train_avp']:.3f} "
            f"test_ap={test_metrics['agg_test_avp']:.3f}"
        )

    final_train_auc = train_metrics["agg_train_auc"]
    _, final_test_labs, final_test_scores = test_classifier(
        classifier, test_loader, criterion, mean_feats, std_feats
    )

    return classifier.state_dict(), final_test_labs, final_test_scores, final_train_auc
...
def nested_leave_one_out(cfg):
    ...
    for pat_id in cfg.dataset.ids:
        ...
        try:
            test_auc = roc_auc_score(final_test_labs, final_test_scores)
        except ValueError:
            test_auc = float("nan")
        log.info(
            f"Patient {pat_id}: final train_auc={final_train_auc:.3f} "
    ...
    per_patient_test_auc = [
        roc_auc_score(dic["ys"], dic["scores"]) for dic in results_dict.values()
    ]
    mean_test_auc = float(np.mean(per_patient_test_auc))
    # Pooled AUC (all patients' predictions concatenated) is more stable than
    # the per-patient mean when positives are rare.
    all_ys = np.concatenate([dic["ys"] for dic in results_dict.values()])
    all_scores = np.concatenate([dic["scores"] for dic in results_dict.values()])
    log.info(
        "=== SUMMARY === "
        f"mean_train_auc={np.mean(train_aucs):.3f} | "
        f"mean_test_auc={mean_test_auc:.3f} | "
        f"pooled_test_auc={roc_auc_score(all_ys, all_scores):.3f}"
    )
